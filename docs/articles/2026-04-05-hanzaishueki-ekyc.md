...
- OpenID Connect for Identity Assurance 1.0（OID4IDA）のトラストフレームワーク `jp_aml` として犯収法要件が国際標準に接続されており、相互運用の基盤が整いつつある
...
2025年2月、警察庁は「犯収法施行規則の一部改正」のパブリックコメントを発出しました（[警察庁公表資料](https://www.npa.go.jp/news/release/2024/20240820gaiyousiryou.pdf)）。改正の核心は以下の通りです。

### 廃止される方式

| 方式       | 内容                         | 廃止理由                                                           |
| ---------- | ---------------------------- | ------------------------------------------------------------------ |
| **ホ方式** | 書類・容貌画像の撮影・送信型 | 偽造書類・AI生成顔画像による不正リスク、FAFTリスク評価基準との乖離 |
...
OID4IDA は `verified_claims` というJSON構造でエンドユーザー情報と「どのように本人確認を行ったか」のメタデータをバンドルして伝達します。日本の犯収法が **`jp_aml` トラストフレームワーク**として同仕様に定義されており、日本国内のeKYCが国際的な相互運用の枠組みに接続されています。
...
- **生態系的に**: マイナンバーカードをハブとしたデジタルアイデンティティ基盤（JPKIおよびOID4IDA `jp_aml`）が民間サービスに本格的に普及する
