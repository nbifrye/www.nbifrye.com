...
RFC 9068 は、トークン情報を暗号署名付き JWT に埋め込むことで、リソースサーバーが**認可サーバーへの追加呼び出しなしにローカル検証**できるようにします。FAPI 2.0、OpenBanking、DPoP（RFC 9449）、PAR（RFC 9126）など、現代の高セキュリティ OAuth 実装の多くが RFC 9068 を前提とします。
...
RFC 9068 以前の大きな問題として **Token Confusion Attack** がありました。OpenID Connect の ID Token と OAuth 2.0 の Access Token はどちらも JWT であり、`typ` 値が区別されていなかったため、攻撃者が ID Token を Access Token として提示できる脆弱性がありました。
...
**業界標準での採用**: FAPI 2.0 Security Profile は RFC 9068 を前提とした上で DPoP（RFC 9449）と PAR（RFC 9126）を追加します。英国 OpenBanking 標準や EU PSD2 対応 API はこの組み合わせを採用しています。
...
| [FAPI 2.0 Security Profile](https://openid.net/specs/fapi-security-profile-2_0-final.html) | RFC 9068 を基盤とした金融 API セキュリティプロファイル |
