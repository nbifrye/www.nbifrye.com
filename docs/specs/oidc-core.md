...
**注意**: トークンがブラウザ URL 経由で露出するリスクがある。現在は **非推奨**。SPA には Authorization Code Flow + PKCE（RFC 7636）を使うことが推奨される。
...
仕様には定義されているが、現実のユースケースで Implicit Flow を新規採用する理由はない。Authorization Code Flow + PKCE の組み合わせが Public Client（SPA、ネイティブアプリ）でも安全に使えることが確立されており、多くのセキュリティガイドライン（OAuth 2.0 Security Best Current Practice）が明示的に Implicit Flow を非推奨としている。
