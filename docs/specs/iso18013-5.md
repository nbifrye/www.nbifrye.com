...
iOS 16 から ISO/IEC 18013-5 準拠の mDL をネイティブサポート。Secure Enclave にデバイス鍵を格納し、Apple も発行局も使用状況をトラッキングできない設計を採用しています ([Apple Security Guide](https://support.apple.com/guide/security/security-of-ids-in-apple-wallet-secb569bf393/web))。QR + BLE のDevice Retrievalフローを実装。
