...
## Workload Proof Token（WPT）と S2S プロトコル
...
**代替アプローチ**: WPT の代わりに RFC 9421 HTTP Message Signatures を使うオプションも定義されており、TLS 層での mTLS（WIC を使用）も有効な手段です。
...
| `draft-ietf-wimse-s2s-protocol`（S2S プロトコル）    | -06        | 2026 年 2 月  | Standards Track |
| `draft-ietf-wimse-workload-identity-practices`（BP） | -03        | 2025 年 10 月 | Informational   |

アーキテクチャドキュメントは 2026 年 7 月の RFC 化を目標としており、Standards Track のドラフト群はその後に続く予定です。IETF OAuth WG・SCIM WG・RATS WG との協調も進んでいます。
...
- [draft-ietf-wimse-s2s-protocol — S2S Protocol](https://datatracker.ietf.org/doc/draft-ietf-wimse-s2s-protocol/)
