...
Kubernetes 1.22 で GA となった Bound Service Account Token（TokenRequest API）は、Pod 専用の短命 JWT を発行する仕組みです。v1.22 以降ではデフォルトの SA トークン発行方式が長命 Secret から TokenRequest ベースに切り替わり、従来の長命 SA トークン（Secret としてクラスター内に常駐）の問題を解決します。
...
当初の `draft-ietf-wimse-s2s-protocol`（rev-07 で Expired）は機能別に複数のドラフトに分割・再編されました（2025年後半）。

| ドラフト                                                                                                                          | 内容                                                              | ステータス      |
| --------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------- |
| [draft-ietf-wimse-workload-creds](https://datatracker.ietf.org/doc/draft-ietf-wimse-workload-creds/)                              | WIT（Workload Identity Token）形式の定義                          | WG Draft        |
| [draft-ietf-wimse-wpt](https://datatracker.ietf.org/doc/draft-ietf-wimse-wpt/)                                                    | WPT（Workload Proof Token）形式の定義                             | WG Draft        |
| [draft-ietf-wimse-http-signature](https://datatracker.ietf.org/doc/draft-ietf-wimse-http-signature/)                              | HTTP Message Signatures を使ったサービス間認証                    | WG Draft        |
| [draft-ietf-wimse-mutual-tls](https://datatracker.ietf.org/doc/draft-ietf-wimse-mutual-tls/)                                      | Mutual TLS によるサービス間認証                                   | WG Draft        |
| [draft-ietf-wimse-identifier-02](https://datatracker.ietf.org/doc/draft-ietf-wimse-identifier/)                                   | ワークロード識別子 URI の標準化                                   | WG Draft rev.02 |
...
Workload Proof Token (WPT): typ="wpt+jwt"（秘密鍵所持の証明）
  - wth: WIT のハッシュ
  - aud: ターゲット URI
```
...
**`typ` ヘッダーの省略に注意**：WIMSE は WIT に `typ: "wit+jwt"`、WPT に `typ: "wpt+jwt"` を必須とします（[draft-ietf-wimse-workload-creds](https://datatracker.ietf.org/doc/draft-ietf-wimse-workload-creds/)）。`typ` を省略すると異なるコンテキストの JWT が別サービスで受理される「コンテキスト混乱攻撃」のリスクが生じます。Kubernetes SA トークンも同様に `aud` クレームで用途を限定することが推奨されます。
...
WIMSE の各ドラフトが RFC 化されれば、SPIFFE ID をベースにした統一的なワークロード認証プロトコルが実現します。[IETF WIMSE WG](https://datatracker.ietf.org/wg/wimse/about/) の標準化進捗を継続的に追うことを推奨します。
...
- [draft-ietf-wimse-workload-creds](https://datatracker.ietf.org/doc/draft-ietf-wimse-workload-creds/) — WIT（Workload Identity Token）形式の定義
- [draft-ietf-wimse-wpt](https://datatracker.ietf.org/doc/draft-ietf-wimse-wpt/) — WPT（Workload Proof Token）形式の定義
- [draft-ietf-wimse-workload-identity-practices](https://datatracker.ietf.org/doc/draft-ietf-wimse-workload-identity-practices/) — ワークロードアイデンティティのベストプラクティス
- [draft-ietf-wimse-identifier](https://datatracker.ietf.org/doc/draft-ietf-wimse-identifier/) — ワークロード識別子 URI
