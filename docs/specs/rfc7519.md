...
RFC 7519 は 2015 年 5 月に Proposed Standard として公開され、現在は RFC 7797 および RFC 8725 によって更新されています（[RFC 7519](https://www.rfc-editor.org/rfc/rfc7519)）。
...
**対策**: 検証実装では `none` を明示的に禁止する。RFC 8725 は「ライブラリは `none` を生成・受容してはならない、ただしアプリケーションが明示的に要求した場合を除く」と規定しています（[RFC 8725 Section 3.6](https://www.rfc-editor.org/rfc/rfc8725#section-3.6)）。
...
| SD-JWT（選択的開示）             | RFC 9558 / SD-JWT VC                         |
...
- [RFC 9558 — SD-JWT](https://www.rfc-editor.org/rfc/rfc9558)
