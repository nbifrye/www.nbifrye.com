...
> **このファイルは Claude Code 向けの拡張版**。ワークフローの正定義は `.agents/skills/work/SKILL.md` にある。
> 以下の最適化メモは Claude Code 専用（Task ツール・モデル指定など）であり、他エージェントは `.agents/skills/work/SKILL.md` を参照すること。
...
以下に該当する場合のみ対象ファイルを Read → Edit する：
...
_このコマンドは毎回実行するたびに新しいコンテンツを生み出し、自らも進化する。
