...
> **重要**: フロントマターに `tags` フィールドを追加してはならない。
> `レビュー済み` タグは **review-mode でのみ** 付与される。write-mode では絶対に追加しないこと。
...
> **禁止**: このステップでの修正に関わらず、`レビュー済み` タグを付与してはならない。
...
> **禁止**: 校正完了後も `レビュー済み` タグを付与してはならない。タグ付与は review-mode の専任。
