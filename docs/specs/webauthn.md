...
| Level 2    | Recommendation (2021年4月)           | 認証器の大規模証明（AAGUID）、拡張機能の追加               |
| Level 3    | Candidate Recommendation (2026年1月) | Passkey サポート強化、シグナル API、バックアップ状態フラグ |
...
アテステーションは「この認証器は信頼できるメーカーのものである」ことを証明する仕組みです。8種類のアテステーション形式が定義されています。
