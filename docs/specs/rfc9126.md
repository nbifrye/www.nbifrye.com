...
JAR（JWT-Secured Authorization Request, RFC 9101）はリクエストを JWT に署名（必須）し、必要に応じて暗号化することで一部の問題に対処しましたが、JWT 本体はやはりブラウザ経由で送信されます。
...
`request_uri` の有効期限は認可サーバーが任意に設定できますが、典型的には 5〜600秒程度が想定されます（[RFC 9126 Section 2.2](https://www.rfc-editor.org/rfc/rfc9126#section-2.2)）。ワンタイム使用については、クライアントは `request_uri` を必ず 1 回のみ使用しなければなりません（MUST）。認可サーバー側はワンタイム使用を実施することを推奨されていますが（SHOULD）、強制ではありません。これはリプレイ攻撃の時間窓を最小化するためです。
...
`request_uri` の有効期限は認可サーバーが任意に設定しますが（典型的には数十秒から数分程度）、実際には短めに設定されるケースが多いです。PAR リクエスト送信から認可エンドポイントへのリダイレクトまでを速やかに行う実装が必要です。ネットワーク遅延・リトライ処理で有効期限切れになるケースに注意してください。
