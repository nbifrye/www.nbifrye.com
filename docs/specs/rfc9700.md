...
RFC 9700 はリフレッシュトークンに対して以下を要求します（[RFC 9700 Section 4.14](https://www.rfc-editor.org/rfc/rfc9700.html#section-4.14)）。
...
認可エンドポイントを iframe 内に埋め込まれ、ユーザーが意図せず認可操作を行わされる攻撃です（[RFC 9700 Section 4.16](https://www.rfc-editor.org/rfc/rfc9700.html#section-4.16)）。

対策として認可サーバーは `X-Frame-Options: DENY` または CSP `frame-ancestors 'none'` ヘッダーを送出すべきです。

### 10. 307 リダイレクト問題

認可サーバーがトークンエンドポイントへのリクエストを 307（Temporary Redirect）でリダイレクトすると、ブラウザはリクエストボディ（`client_secret` 等）を含めたまま転送します（[RFC 9700 Section 4.12](https://www.rfc-editor.org/rfc/rfc9700.html#section-4.12)）。
...
- クライアントアプリケーションがユーザーの認証情報（パスワード）を直接扱う
- MFA（多要素認証）に対応できない
