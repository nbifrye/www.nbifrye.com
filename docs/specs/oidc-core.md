...
**注意**: トークンがブラウザ URL フラグメント経由で露出するリスクがある。OIDC の Implicit Flow 自体は OpenID Foundation によって正式に非推奨とはされていないが、セキュリティ上の問題から SPA には Authorization Code Flow + PKCE（RFC 7636）を使うことが推奨される。なお混同しやすい点として、OAuth 2.0 の Implicit Grant（`response_type=token`）は RFC 9700 (OAuth 2.0 Security Best Current Practice) Section 2.1.2 で MUST NOT と明記されており、こちらは正式に廃止されている。
...
仕様には定義されているが、現実のユースケースで Implicit Flow を新規採用する理由はない。Authorization Code Flow + PKCE の組み合わせが Public Client（SPA、ネイティブアプリ）でも安全に使えることが確立されており、セキュリティ上の理由から利用を避けることが推奨される。

ただし、ここで注意すべき区別がある。**正式に廃止（MUST NOT）** とされているのは **OAuth 2.0 の Implicit Grant**（`response_type=token`）であり、RFC 9700 Section 2.1.2 がその根拠だ。一方、**OIDC Core の Implicit Flow**（`response_type=id_token` または `response_type=id_token token`）は OpenID Foundation によって正式に非推奨とは宣言されていない。OAuth 2.0 Security Best Current Practice が扱う対象はあくまで OAuth 2.0 のグラントタイプであり、OIDC 固有のフローの廃止は別途 OpenID Foundation が決定する事項だ。この区別を混同すると、標準仕様の理解として不正確になる。
