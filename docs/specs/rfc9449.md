...
リソースサーバは以下をすべて検証する必要があります：

1. DPoP JWTの署名（JOSEヘッダ内の`jwk`公開鍵を使用）
2. `typ`が`"dpop+jwt"`であること
3. `htm`がリクエストのHTTPメソッドと一致すること
4. `htu`がリクエストのURIと一致すること（クエリ・フラグメントを除く）
5. `iat`が現在時刻の許容範囲内であること
6. `jti`が未知の値（リプレイでない）であること
7. `ath`がAuthorizationヘッダのトークンのSHA-256と一致すること
8. トークンの`cnf.jkt`がDPoP ProofのJWKのThumbprintと一致すること
...
- **[RFC 9126 (PAR)](https://www.rfc-editor.org/rfc/rfc9126)**: `dpop_jkt`パラメータとの組み合わせで認可コードへの鍵バインディングを強化
