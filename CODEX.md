...
詳細は `.codex/skills/work/style-guide.md` を参照。要点:
...
PostToolUse フックにより、Write/Edit ツール使用後は自動的に実行されます（`.codex/settings.json`）。
...
`.codex/skills/work/topics.md` にトピックマトリクスを管理しています。
...
- 基盤ファイル（CODEX.md / topics.md / style-guide.md / config.mts）の改善も同じコミットに含めてよい
