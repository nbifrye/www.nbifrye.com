...
OID4IDA は `verified_claims` というJSON構造でエンドユーザー情報と「どのように本人確認を行ったか」のメタデータをバンドルして伝達します。日本の犯収法が **`jp_aml` トラストフレームワーク識別子**として同仕様の例示で参照されており、日本国内のeKYCが国際的な相互運用の枠組みに接続されています。
