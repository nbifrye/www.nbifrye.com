...
- **仕様ステータスを一次情報で確認する（精度チェック）**: 「非推奨」「廃止」「MUST NOT」等の強い評価を書く場合は、必ず該当 RFC/仕様の当該セクションを根拠として示す。特に **複数の仕様に類似概念が存在する場合**（例: OAuth 2.0 Implicit Grant vs OIDC Implicit Flow、RFC 6749 vs RFC 9700）は、**どの仕様のどのセクションにおける評価かを明確に区別する**。サブエージェントの要約や二次情報だけで断定的な記述をしないこと。
