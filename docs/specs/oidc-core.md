...
5. JWT 署名を OP の公開鍵（JWK）で検証する。Authorization Code Flow の場合、Token エンドポイントとの直接通信（TLS）が担保されていれば、署名検証の代わりに TLS サーバー検証を使ってもよい（MUST NOT use `none` unless the flow returns no ID Token from Authorization Endpoint）
