...
OpenID Federation 1.0 は、複数の組織が参加する大規模な OpenID Connect エコシステムにおいて、参加者間の信頼関係を自動的に確立するための仕様です。OpenID Foundation が策定し、2026 年 2 月に Final Specification として公開されました ([OpenID Federation 1.0](https://openid.net/specs/openid-federation-1_0.html))。
...
EBSI は TAO（Trusted Accreditation Organisation）階層が Trust Chain 構造に対応しており、DID（Decentralized Identifier）を独自拡張として組み合わせる形で OpenID Federation の統合を進めています。
