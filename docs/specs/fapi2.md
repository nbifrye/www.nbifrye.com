...
FAPI 2.0（Financial-grade API Security Profile 2.0）は、OAuth 2.0 をベースに金融グレードの高セキュリティ要件を定めた OpenID Foundation の仕様です。2025年2月22日に Final として正式承認され、Open Banking・決済・オープンファイナンスなどの高リスク環境での API アクセス制御標準として国際的に採用が進んでいます。
...
| 地域・組織                | 採用状況                                                             |
| ------------------------- | -------------------------------------------------------------------- |
| **英国 Open Banking**     | FAPI 2.0 への段階的移行進行中。複数の金融機関が FAPI 認定取得済み    |
...
| **Authlete 2.3**          | 2025 年 7 月に FAPI 2.0 Final の OpenID 認定取得（業界初）           |
