...
**注意**: トークンがブラウザ URL フラグメント経由で露出するリスクがある。OIDC Core 仕様は Implicit Flow を正式に非推奨とはしていないが、Authorization Code Flow + PKCE の方がセキュリティ上優れているため、新規実装では使用しないことが推奨される。なお、混同しやすい点として **OAuth 2.0 の Implicit Grant**（`response_type=token`）は RFC 9700 により「MUST NOT be used」と明示的に廃止されており、これは OIDC Implicit Flow とは別の話である。
...
### Implicit Flow は使わない（ただし廃止ではない）

仕様には定義されているが、現実のユースケースで Implicit Flow を新規採用する理由はない。Authorization Code Flow + PKCE の組み合わせが Public Client（SPA、ネイティブアプリ）でも安全に使えることが確立されており、実質的に置き換えられている。

ここで重要な区別がある。**廃止（MUST NOT be used）とされているのは OAuth 2.0 の Implicit Grant**（RFC 6749 Section 4.2、`response_type=token`）であり、RFC 9700 がこれを明示している。**OIDC の Implicit Flow**（`response_type=id_token` / `response_type=id_token token`）は OIDC Core 仕様において正式に廃止されていない。ただし、トークンが URL フラグメントに露出するリスク構造は同様に存在するため、新規実装では使わない判断が合理的だ。OAuth 2.0 Implicit Grant と OIDC Implicit Flow を同一視する記述を見かけたら注意深く読む必要がある。
