...
- **Architecture Reference Framework（ARF）v2.7.3** が本記事執筆時点の参照バージョンであり、PID・EAA・Wallet Unit Attestation（WUA）・信頼リストという4層の信頼モデルを規定する。
...
ARF は法的拘束力を持たず「参照実装のガイダンス」という位置付けだが、大規模パイロットの技術設計から CIR 草案策定まで実質的な標準として機能してきた。本記事は **v2.7.3**（2024年11月リリース）を参照する。v2.8.0（2025年2月）以降もリビジョンが継続しており、最新版は [GitHub リポジトリ](https://github.com/eu-digital-identity-wallet/eudi-doc-architecture-and-reference-framework) で確認されたい。
...
| **DC4EU**     | 教育・社会保障の QEAA クロスボーダー交換                       | 26（101パートナー）                                                   | 本番環境への移行ブループリントを提示         |
...
DC4EU は 101 のパートナーと 26 カ国での実証を経て、教育証明書・社会保障属性の QEAA 交換の技術的・運用的ブループリントを完成させた。
...
eIDAS 2.0（[Regulation (EU) 2024/1183](https://eur-lex.europa.eu/legal-content/EN/TXT/?uri=CELEX%3A32024R1183)）は、**2027年末**までに以下の分野の民間サービス提供者が EUDI Wallet 対応を行うことを義務付けている。
...
EUDI Wallet のエコシステムは法的基盤（eIDAS 2.0）・技術仕様（ARF v2.7.3）・委任法令（CIR の3波）が概ね整備され、実装フェーズに移行しつつある。大規模パイロットの知見は今後の開発に重要なインプットを提供した。
